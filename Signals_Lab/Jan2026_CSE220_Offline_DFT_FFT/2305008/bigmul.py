...
Usage (the command line is already wired up for you):
    ...
    python3 bigmul.py inputs/1.txt --engine dft --out-dir outputs/1
    python3 bigmul.py inputs/2.txt --engine dft --out-dir outputs/2
    python3 bigmul.py inputs/3.txt --engine fft --out-dir outputs/3
    python3 bigmul.py inputs/4.txt --engine fft --out-dir outputs/4
...
def to_limbs(text, base_digits=BASE_DIGITS):
    ...
    limbs = []
    length = len(text)
    for end in range(length, 0, -base_digits): # end is exclusive
...
def from_limbs(sign, limbs, base_digits=BASE_DIGITS):
    ...
    # TODO: implement this function
    base = 10 ** base_digits
    res = [] # limbs after carry propagation, converted to string, unpadded
    carry = 0
    for limb in limbs:
        cur = limb + carry
        res.append(str(cur % base))
        carry = cur // base

    while carry > 0: # carry can overflow from highest limb
        res.append(str(carry % base))
        carry //= base

    res_padded = [res[-1]]
    for limb in reversed(res[:-1]): # all except the most significant limb must be padded with leading zeros
        res_padded.append(limb.zfill(base_digits))

    res_str = ''.join(res_padded).lstrip('0') or '0'
    if res_str == '0':
        return '0'
...
def multiply_transform(a, b, engine):
    ...
    a_trans = engine.transform(a_padded)
    b_trans = engine.transform(b_padded)
    c_trans = a_trans * b_trans

    c = engine.inverse(c_trans)
    limbs = np.round(c.real).astype(np.int64)
    return limbs, N
...
def multiply_schoolbook(a, b):
    ...
    res = np.zeros(len_a + len_b - 1, dtype=np.int64)
    for i in range(len_a):
        for j in range(len_b):
            res[i + j] += a[i] * b[j]
    return res, len_a + len_b - 1
...
def multiply(text_a, text_b, method):
    ...
    if method == "fft":
        ...
        limbs_c, N = multiply_transform(limbs_a, limbs_b, engine)
    elif method == "dft":
        engine = DFTAnalyzer()
        limbs_c, N = multiply_transform(limbs_a, limbs_b, engine)
    elif method == "schoolbook":
        limbs_c, N = multiply_schoolbook(limbs_a, limbs_b)  
    
    product = from_limbs(sign, limbs_c)
...
def run_single(path, method, out_dir):
    ...
    verdict = "MATCH" if product_str == expected else "MISMATCH"

    digits_a = len(text_a.lstrip('-+'))
    digits_b = len(text_b.lstrip('-+'))
    digits_product = len(product_str.lstrip('-'))

    report_lines = [
        f"Task A -- big-integer multiplication by spectral convolution",
        f"input file          : {path}",
        f"method              : {method}",
        ...
        f"digits of product   : {digits_product}",
