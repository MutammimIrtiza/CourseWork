...
def transform_2d(plane, engine):
    ...
    P, Q = plane.shape
    row_fft = np.zeros((P, Q), dtype=np.complex128)
    for row in range(P):
        row_fft[row, :] = engine.transform(plane[row, :])
    col_fft = np.zeros((P, Q), dtype=np.complex128)
    for col in range(Q):
        col_fft[:, col] = engine.transform(row_fft[:, col])
    return col_fft
...
def inverse_2d(spectrum, engine):
    ...
    P, Q = spectrum.shape
    row_ifft = np.zeros((P, Q), dtype=np.complex128)
    for row in range(P):
        row_ifft[row, :] = engine.inverse(spectrum[row, :])
    col_ifft = np.zeros((P, Q), dtype=np.complex128)
    for col in range(Q):
        col_ifft[:, col] = engine.inverse(row_ifft[:, col])
    return col_ifft
...
def convolve_plane(plane, kernel, engine, circular=False):
    ...
    if not circular:
        ...
        padded_plane = np.zeros((P, Q), dtype=np.float64)
        padded_kernel = np.zeros((P, Q), dtype=np.float64)

        # at top left 
        padded_plane[:H, :W] = plane
        padded_kernel[:kh, :kw] = kernel

        trans_plane = transform_2d(padded_plane, engine)
        trans_kernel = transform_2d(padded_kernel, engine)
        trans_prod = trans_plane * trans_kernel

        res = inverse_2d(trans_prod, engine).real
        r_start = kh // 2
        c_start = kw // 2

        return res[r_start:r_start + H, c_start:c_start + W]

    else:
        P, Q = H, W
        padded_kernel = np.zeros((P, Q), dtype=np.float64)
        padded_kernel[:kh, :kw] = kernel
        rolled_kernel = np.roll(padded_kernel, (- (kh // 2), - (kw // 2)), axis=(0, 1))

        trans_plane = transform_2d(plane, engine)
        trans_kernel = transform_2d(rolled_kernel, engine)
        trans_prod = trans_plane * trans_kernel

        return inverse_2d(trans_prod, engine).real
...
def convolve_image(image, kernel, engine, circular=False):
    ...
    if image.ndim == 2:  
        ...
        channels = [convolve_plane(image[:, :, c], kernel, engine, circular=circular) for c in range(image.shape[2])]
...
def run_single(path, kernel_name, param, engine_name, out_dir, gray=False):
    ...
    max_err = np.max(np.abs(spectral_crop - direct_crop))
    ...
    min_q = W + kw - 1
    if engine.name == "fft":
        P = next_power_of_two(min_p)
        Q = next_power_of_two(min_q)
    else:
        P = min_p
        Q = min_q

    report_lines = [
        f"Task B -- image blurring by 2D spectral convolution",
        f"image path          : {path}",
        f"image shape         : {image.shape}",
        f"kernel name / shape : {kernel_name} / {kernel.shape}",
        f"engine              : {engine_name}",
        f"linear conv size    : ({min_p}, {min_q})",
        f"transform size used : ({P}, {Q})",
        f"verification max err: {max_err:.2e}"
    ]
    write_report(os.path.join(out_dir, "report.txt"), report_lines)
    print(f"Verification max err: {max_err:.2e}")
